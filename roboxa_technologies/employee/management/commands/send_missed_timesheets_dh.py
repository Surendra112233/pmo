...
class Command(BaseCommand):
    help = 'Send consolidated missed timesheet summaries to delivery heads'
    ...
    def handle(self, *args, **options):
        ...
        date_list = [
            ...
            if (start_date + timedelta(days=i)).weekday() < 5  # Mon-Fri only
        ]

        # Data structure: dh_name -> {
        #   'email': email,
        #   'projects': {
        #       project_description: {
        #           employee_name: missed_dates_count
        #       }
        #   }
        # }
        dh_data = {}
        ...
        for emp in employees:
            ...
            if missed_dates:
                ...
                for pt in project_teams:
                    ...
                    dh_name = project.delivery_head
                    if not dh_name:
                        continue

                    dh_user = UserProfile.objects.filter(name__iexact=dh_name).first()
                    if not dh_user or not dh_user.email:
                        self.stdout.write(f"No email found for Delivery Head: {dh_name} (employee: {emp.name})")
                        continue

                    if dh_name not in dh_data:
                        dh_data[dh_name] = {
                            'email': dh_user.email,
                            'projects': defaultdict(dict)
                        }

                    emp_missed_count = len(missed_dates)
                    dh_data[dh_name]['projects'][project.project_description][emp.name] = emp_missed_count

        # Send consolidated emails
        for dh_name, data in dh_data.items():
            email = data['email']
            ...
            for project_name, employees in projects.items():
                ...
                for emp_name, missed_count in employees.items():
            ...
            self.stdout.write(f"Consolidated email sent to Delivery Head {dh_name}")
